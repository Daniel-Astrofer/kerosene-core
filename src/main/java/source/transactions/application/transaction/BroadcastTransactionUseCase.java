...
public class BroadcastTransactionUseCase {
    ...
    public TransactionResponseDTO broadcast(
        ...
        try {
            ...
            log.warn("Failed to save broadcast history for txid={}: {}", txid, ex.getMessage());
        }

        try {
            transactionNotificationPort.notifySenderBroadcast(userId, amount);
        } catch (RuntimeException ex) {
            log.warn("Failed to notify sender for txid={}: {}", txid, ex.getMessage());
        }
        ...
        if (toAddress != null && !toAddress.isBlank()) {
            try {
                ...
                log.warn("Failed to notify recipient for txid={}: {}", txid, ex.getMessage());
            }
        }
