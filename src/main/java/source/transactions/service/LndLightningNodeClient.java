...
public class LndLightningNodeClient implements LightningInvoiceGateway, LightningPaymentGateway, LightningClient {

    private static final Logger log = LoggerFactory.getLogger(LndLightningNodeClient.class);
    ...
    public void init() {
        ...
        if (tlsEnabled) {
            ...
            log.warn("[LndLightningNodeClient] Plaintext gRPC is enabled only for local development.");
        ...
        log.info("[LndLightningNodeClient] Connected to LND gRPC at {}:{}.", host, port);
    }
    ...
    public void importWatchOnlyPublicKey(byte[] publicKey, String expectedAddress) {
        ...
        try {
            walletKitBlockingStub.importPublicKey(ImportPublicKeyRequest.newBuilder()
                    ...
                    .build());
            log.info("[LndLightningNodeClient] Imported watch-only public key for addressRef={}.",
                    LogSanitizer.fingerprint(expectedAddress));
        } catch (StatusRuntimeException ex) {
            if (isAlreadyExists(ex)) {
                log.debug("[LndLightningNodeClient] Watch-only key for addressRef={} was already imported.",
                        LogSanitizer.fingerprint(expectedAddress));
                return;
            }
            throw ex;
