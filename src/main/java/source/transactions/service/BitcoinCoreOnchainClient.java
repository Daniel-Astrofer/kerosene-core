...
@Component("bitcoinCoreOnchainClient")
@ConditionalOnProperty(prefix = "lightning.lnd", name = "enabled", havingValue = "true")
public class BitcoinCoreOnchainClient implements WatchOnlyAddressImportPort {

    private static final Logger log = LoggerFactory.getLogger(BitcoinCoreOnchainClient.class);
    ...
    public BitcoinCoreOnchainClient(
    ...
    public void init() {
        ...
        if (tlsEnabled) {
            ...
            log.warn("[BitcoinCoreOnchainClient] Plaintext gRPC is enabled only for local development.");
        ...
        log.info("[BitcoinCoreOnchainClient] Connected to LND gRPC at {}:{}.", host, port);
    }
    ...
    public void importWatchOnlyPublicKey(byte[] publicKey, String expectedAddress) {
        ...
        try {
            walletKitBlockingStub.importPublicKey(ImportPublicKeyRequest.newBuilder()
                    ...
                    .build());
            log.info("[BitcoinCoreOnchainClient] Imported watch-only public key for addressRef={}.",
                    LogSanitizer.fingerprint(expectedAddress));
        } catch (StatusRuntimeException ex) {
            if (isAlreadyExists(ex)) {
                log.debug("[BitcoinCoreOnchainClient] Watch-only key for addressRef={} was already imported.",
                        LogSanitizer.fingerprint(expectedAddress));
                return;
            }
            throw ex;
