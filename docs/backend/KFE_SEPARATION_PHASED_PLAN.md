...
- Gradle multi-module. Done for first physical split: `settings.gradle.kts` includes `:kerosene-contracts`…
- `:kerosene-core`
- `:kfe-service` — holds `com.kerosene.kfe`
- `:kerosene-contracts`
- Mesh client lives in kfe vaultmesh adapter (not a revived mpc-sidecar client as primary)
- Move KFE domain into `kfe-service`. Done under `backend/kerosene/kfe-service/.../com/kerosene/kfe`.
...
- Separar testes por módulo e manter um pacote de testes de contrato. Iniciado: `:kfe-service:test`, `:kerosene-contracts:build`…
- O módulo `:kerosene-contracts` compila isolado, sem Spring/JPA/implementações. Done.
- `:kerosene-shared` builds alone; no impl-package deps. Done.
- `:kfe-service` builds/tests alone and holds `com.kerosene.kfe`. Done for first physical cut.
...
- Separar Gradle em módulos. **Avançado com `:kerosene-contracts`, `:kerosene-shared` e `:kfe-service`.**
...
- O módulo `:kerosene-shared` compila isolado e não importa pacotes de implementação.
- O módulo `:kfe-service` compila/testa isolado e o app principal compila consumindo-o como dependência.
